...
class VideoDataset_images_with_motion_features(data.Dataset):
    ...
    def __init__(self, data_dir, data_dir_3D ,filename_path, transform, database_name, crop_size, feature_type):
        ...
        if database_name == 'KoNViD-1k':
            ...
            self.video_names = video_names.strip()
            self.score = score
...
class VideoDataset_images_VQA_dataset_with_motion_features(data.Dataset):
    """Read data from the original dataset for feature extraction"""
    def __init__(self, data_dir, data_dir_3D ,filename_path, transform, database_name, crop_size, feature_type, exp_id, state):
        ...
        if database_name == 'KoNViD-1k':
            ...
            for i in index:
                video_names.append(dataInfo['video_names'][i].strip())
                score.append(dataInfo['scores'][i][0])
            self.video_names = video_names
            self.score = score
...
class VideoDataset_NR_SlowFast_feature(data.Dataset):
    ...
    def __init__(self, data_dir, filename_path, transform, resize, database_name):
        ...
        if database_name == 'KoNViD-1k':
            ...
            self.video_names = filename_path
    ...
    def __getitem__(self, idx):
        ...
        if video_clip < video_clip_min and video_clip != 0:
            for i in range(video_clip, video_clip_min):
                transformed_video_all.append(transformed_video_all[video_clip - 1])

        if video_clip == 0:
            print("aaaaaaa",video_name_str)

        return transformed_video_all, video_name_str

class VideoDataset_NR_SlowFast_feature1(data.Dataset):
    """Read data from the original dataset for feature extraction"""
    def __init__(self, data_dir, filename_path, transform, resize, database_name, state):
        super(VideoDataset_NR_SlowFast_feature1, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        return transformed_video_all, video_name_str, video_score

class VideoDataset_VQA_Swin_features(data.Dataset):
    """Read data from the original dataset for feature extraction"""

    def __init__(self, data_dir, data_dir_3D, filename_path, transform, database_name, feature_type, state):
        super(VideoDataset_VQA_Swin_features, self).__init__()
        ...
        if database_name == 'KoNViD-1k':
            ...
            for i in range(n_video):
                video_names.append(infos[i].get('filename'))
                score.append(infos[i].get('label'))
            self.video_names = video_names
            self.score = score
    ...
    def __getitem__(self, idx):
        ...
        video_channel = 3
        #video_height_crop = 256
        #video_width_crop = 256
        video_height_crop = 512
        video_width_crop = 480
        video_length_read = 8
...
class VideoDataset_LSVQ_Swin_features(data.Dataset):
    """Read data from the original dataset for feature extraction"""

    def __init__(self, data_dir, data_dir_3D, filename_path, transform, database_name, feature_type):
        super(VideoDataset_LSVQ_Swin_features, self).__init__()
    ...
    def __getitem__(self, idx):
        if self.database_name == 'LSVQ_train' or self.database_name == 'LSVQ_test' or self.database_name == 'LSVQ_test_1080p':
            video_name = self.video_names[idx] + '.mp4'
            video_name_str = video_name[:-4]
