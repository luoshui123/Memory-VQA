...
def main(config):
    ...
    if config.database == 'KoNViD-1k':
        ...
        datainfo_test = 'data/KoNViD-1k_data_modified.mat'

        transformations_test = transforms.Compose([transforms.Resize([resize, resize]), transforms.ToTensor(), \
                                                   transforms.Normalize(mean=[0.45, 0.45, 0.45],
                                                                        std=[0.225, 0.225, 0.225])])

        trainset = VideoDataset_NR_SlowFast_feature(videos_dir, datainfo_test, transformations_test, resize,
                                                    'KoNViD-1k')

    elif config.database == 'youtube_ugc':
        videos_dir = '../DataSet/Youtube/original_videos_h264'
        datainfo_test = 'data/Youtube_ugc.txt'

        train_infos, val_infos = train_test_split(datainfo_test)
        ...
        trainset = VideoDataset_NR_SlowFast_feature(videos_dir, video_names, transformations_test, resize, 'other')
        ...
        testset = VideoDataset_NR_SlowFast_feature(videos_dir, video_names, transformations_test, resize, 'other')

    elif config.database == 'Live_VQC':
        videos_dir = '../DataSet/Video'
        datainfo_test = 'data/LIVE_VQC_data.txt'
    ...
    elif config.database == 'Divide_3k':
        transformations_test = transforms.Compose([transforms.Resize([resize, resize]), transforms.ToTensor(), \
                                                   transforms.Normalize(mean=[0.45, 0.45, 0.45],
                                                                        std=[0.225, 0.225, 0.225])])
        videos_dir = '../DataSet/DIVIDE_3k/videos'
        datainfo_train = 'data/Divide_3k_train.txt'

        train_infos, _ = train_test_split(datainfo_train, 1)
        n_video = len(train_infos)
        video_names = []
        for i in range(n_video):
            video_names.append(train_infos[i].get('filename'))
        ...
        datainfo_test = 'data/Divide_3k_test.txt'

        _, val_infos = train_test_split(datainfo_test, 0)
        n_video = len(val_infos)
        video_names = []
        for i in range(n_video):
            video_names.append(val_infos[i].get('filename'))
    ...
    ## dataloader
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=1, shuffle=False, num_workers=config.num_workers)
