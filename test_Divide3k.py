...
def main(args):
    ...
    weights = 'ckpts/StorageUseLiner_epoch_96_SRCC_0.896589.pth'
    ...
    datainfo = 'data/Divide_3k_test.txt'

    train_infos, val_infos = train_test_split(datainfo, 0)

    transformations_test = transforms.Compose(
        [transforms.Resize((512, 480)), transforms.ToTensor(),
         transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    testset = VideoDataset_VQA_Swin_features(videos_dir, data_dir_3D, val_infos, transformations_test, 'Live_VQC',
                                             'SlowFast', 'val')
