...
from data_loader import  VideoDataset_images_with_motion_features, \
    VideoDataset_LSVQ_Swin_features
...
from final_fusion_model_v2_get_attention import swinv2_small_patch4_window8_256 as create_model_v2_att
...
def main(args):
    ...
    if args.freeze_layers:
        for name, para in model.named_parameters():
            if "head" not in name and "layers.2" not in name and "layers.3" not in name :
    ...
    #LSVQ
    trainset = VideoDataset_LSVQ_Swin_features(videos_dir, feature_dir, datainfo_train, transformations_train,
                                                        'LSVQ_train', 'SlowFast')
    testset = VideoDataset_LSVQ_Swin_features(videos_dir, feature_dir, datainfo_test, transformations_train,
                                                       'LSVQ_test', 'SlowFast')
    testset_1080p = VideoDataset_LSVQ_Swin_features(videos_dir, feature_dir, datainfo_test_1080p,
                                                             transformations_train, 'LSVQ_test_1080p',
                                                             'SlowFast')
    ## dataloader
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if test_SRCC > best_test_criterion:
                ...
                save_model_name = args.ckpt_path + '/' + 'StorageUseLiner_epoch_%d_SRCC_%f.pth' % (epoch + 1, test_SRCC)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--print_samples', type=int, default=8000)
